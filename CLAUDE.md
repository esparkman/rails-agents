...
**The ship gate is `bin/ci`, run whole — and it, not you, produces the signoff.** On a Rails 8 CI project (`config/ci.rb` / `ActiveSupport::ContinuousIntegration`), the verification gate before commit-to-ship or merge is a full-green **`bin/ci`**: setup → rubocop → the security scanners (bundler-audit, importmap audit, brakeman) → `bin/rails test` → `bin/rails test:system` → seed replant. `bin/rails test` + a few targeted system tests + rubocop-on-touched-files is **not** the gate — it skips brakeman, the audits, full-tree rubocop, the complete `test:system`, and the replant. On **basecamp/gh-signoff** projects `bin/ci` calls `gh signoff` on full green, stamping the `signoff` status `main` requires. gh-signoff is trust-based by design (no pre-push hooks, no server verification); the trust is precisely that `bin/ci` ran and passed. So **NEVER hand-stamp the `signoff` status** (`gh api .../statuses -f context=signoff -f state=success`, or `gh signoff` without a green `bin/ci`), and **never propose standing up GitHub Actions / cloud CI to "enforce" signoff** — that contradicts the deliberate gh-signoff choice ("You're the CI now"). This owns to `@rails-git-workflow`; `@rails-deployment` treats the same green `bin/ci` as the pre-ship gate.
